...
old_query = """     // Fetching all for this user/bot and sorting in memory to avoid needing a custom composite index
...
new_query = """     const txSnap = await db.collection('transactions')
       .where('botId', '==', ctx.botId)
       .where('telegramId', '==', telegramId)
...
old_docs = """     const sortedDocs = txSnap.docs.sort((a, b) => {
...
new_docs = """     txSnap.docs.forEach((doc, index) => {"""
...
print("History menu patched back to use index")
